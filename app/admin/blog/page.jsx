...
export default function BlogAdminPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "Published":
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-medium shadow-sm">
            <CheckCircle size={14} /> Published
          </span>
        );
      case "Scheduled":
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-100 text-violet-800 text-xs font-medium shadow-sm">
            <Clock size={14} /> Scheduled
          </span>
        );
      case "Draft":
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 text-amber-800 text-xs font-medium shadow-sm">
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 flex-1">
                <div className="bg-white/70 backdrop-blur-sm p-4 sm:p-5 rounded-2xl border border-gray-200/60 shadow-sm hover:shadow transition-shadow">
                  ...
                  <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">{posts.length}</p>
                </div>
                <div className="bg-white/70 backdrop-blur-sm p-4 sm:p-5 rounded-2xl border border-gray-200/60 shadow-sm hover:shadow transition-shadow">
                  <p className="text-xs sm:text-sm text-gray-600">Published</p>
                  <p className="text-2xl sm:text-3xl font-bold text-emerald-600 mt-1">{publishedCount}</p>
                </div>
                <div className="bg-white/70 backdrop-blur-sm p-4 sm:p-5 rounded-2xl border border-gray-200/60 shadow-sm hover:shadow transition-shadow">
                  <p className="text-xs sm:text-sm text-gray-600">Drafts</p>
                  <p className="text-2xl sm:text-3xl font-bold text-amber-600 mt-1">{draftCount}</p>
                </div>
                <div className="bg-white/70 backdrop-blur-sm p-4 sm:p-5 rounded-2xl border border-gray-200/60 shadow-sm hover:shadow transition-shadow">
                  <p className="text-xs sm:text-sm text-gray-600">Scheduled</p>
                  <p className="text-2xl sm:text-3xl font-bold text-violet-600 mt-1">{scheduledCount}</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {["All", "Published", "Draft", "Scheduled"].map((status) => (
                  <button
                    key={status}
                    onClick={() => setFilterStatus(status.toLowerCase() === "all" ? "all" : status)}
                    className={`
              ...
              {filteredPosts.length === 0 ? (
                ...
                filteredPosts.map((post) => (
                  <motion.article
                    key={post.id}
                    variants={itemVariants}
                    className="group relative bg-white rounded-2xl shadow border border-gray-200 overflow-hidden hover:shadow-xl hover:border-sky-200 transition-all duration-300"
                  >
                    {/* Featured badge */}
                    {post.featured && (
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-5 sm:p-6">
