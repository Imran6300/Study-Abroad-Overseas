...
export async function GET(req, { params }) {
  try {
    ...
    // Each sitemap segment covers 50 API pages (~1000 universities)
    const PAGES_PER_SITEMAP = 50;
    const startPage = (sitemapPage - 1) * PAGES_PER_SITEMAP + 1;
    const endPage = sitemapPage * PAGES_PER_SITEMAP;

    // Parallel fetch with concurrency cap to avoid overwhelming the API
    const BATCH_SIZE = 10;
    let allUniversities = [];
    ...
    // Quality filter — only index universities with enough data
    // confidenceScore >= 0.75 ensures good pages get indexed
